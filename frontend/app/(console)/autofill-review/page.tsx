...
import type { AutofillSession } from "@/lib/types";
...
export default async function AutofillReviewPage({
  ...
  let session: AutofillSession | null = null;
  try {
    session = await api<AutofillSession>(
      `/autofill/confirmations/${searchParams.id}`,
    );
  } catch {
    session = null;
  }
  ...
  return (
    <div className="shell space-y-8">
      ...
      {!session ? (
        <div className="card p-8 text-sm text-muted">Session not found.</div>
      ) : (
        <div className="card max-w-2xl p-6 fade-up d1">
          <div className="flex items-center justify-between">
            <p className="font-mono text-xs text-muted">{session.id}</p>
            <Chip className="border-line-strong bg-surface-2 text-gold">
              {session.status}
            </Chip>
          </div>
          <p className="mt-4 break-all font-mono text-xs text-accent">{session.url}</p>
          ...
          <div className="mt-6 rounded-lg border border-line bg-surface-2 p-4 text-xs leading-relaxed text-muted">
